document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DADOS DAS NOTAS ---
    const allCardsData = {
        'C': { name: 'DÓ', imageName: 'DÓ-pauta.png' },
        'D': { name: 'RÉ', imageName: 'RÉ-pauta.png' },
        'E': { name: 'MI', imageName: 'MI-pauta.png' },
        'F': { name: 'FÁ', imageName: 'FÁ-pauta.png' },
        'G': { name: 'SOL', imageName: 'SOL-pauta.png' },
        'A': { name: 'LÁ', imageName: 'LÁ-pauta.png' },
    ...
    function displayBestScores() {
        for (const level in levels) {
            const bestScore = localStorage.getItem(`bestScore-fa-${level}`);
    ...
    function checkAndSaveBestScore() {
        const bestScoreKey = `bestScore-fa-${currentLevel}`;
    ...
    // --- FUNÇÃO showWinScreen COM A LÓGICA RESTAURADA E CORRIGIDA ---
    function showWinScreen() {
        // A ordem correta: primeiro calcula tudo, depois mostra a tela.
        ...
        // Reseta as estrelas para o estado "vazio"
        starElements.forEach(star => star.classList.remove('filled'));

        // Preenche o número correto de estrelas
        for (let i = 0; i < stars; i++) {
            starElements[i].classList.add('filled');
        }

        // Define a mensagem de desempenho (Excelente, etc.)
        winPerformanceText.textContent = performanceMessages[stars];
        
        // Toca o som de vitória
        winSound.play();

        // **AGORA SIM, MOSTRA A TELA DE VITÓRIA**
        winOverlay.classList.add('show');
        
        // Efeito de confete para boas pontuações
        if (stars >= 2) {
            // Usamos um try...catch por segurança, caso o script de confetes não carregue
            try {
    ...
    function returnToMenu() {
        ...
        setTimeout(() => {
            displayBestScores(); // Atualiza os recordes no menu
