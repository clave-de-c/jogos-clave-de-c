document.addEventListener('DOMContentLoaded', () => {
    ...
    const tapFullscreenOverlay = document.getElementById('tap-fullscreen-overlay');
    ...
    // NOVO MAPEAMENTO PARA A CLAVE DE FÁ (Sons Graves)
    const alturasNaPauta = {
        'C': '101px', // Dó: 2º Espaço
        'D': '86px',  // Ré: 3ª Linha
        'E': '71px',  // Mi: 3º Espaço
        'F': '56px',  // Fá: 4ª Linha (Referência da Clave!)
        'G': '41px',  // Sol: 4º Espaço
        'A': '26px',  // Lá: 5ª Linha
        'B': '11px'   // Si: Espaço Superior
    };
    ...
    // --- FULLSCREEN MOBILE ANTI-CORTE ---
    function checkLandscapeFullscreen() {
        if (window.innerWidth > window.innerHeight && window.innerHeight <= 650 && !document.fullscreenElement) {
            tapFullscreenOverlay.classList.remove('hidden');
            tapFullscreenOverlay.classList.add('show');
        } else {
            tapFullscreenOverlay.classList.remove('show');
            tapFullscreenOverlay.classList.add('hidden');
        }
    }

    tapFullscreenOverlay.addEventListener('click', () => {
    ...
    function processarToqueNota(notaTeclada) {
        ...
        if (naturalNotes.includes(notaTeclada) && !audioMuted) {
        ...
        if (indexAtiva !== -1) {
            ...
            if (notasAtivasNaTela.length === 0) setTimeout(spawnGrupo, 300);
        } else {
            lives--;
            resetarCombo();
    ...
    function computarErro(idNota) {
        ...
        if (index !== -1) {
            ...
            if (!audioMuted) new Audio('miss.mp3').play().catch(() => {});
            verificarDerrota();
            if (!gameIsOver && notasAtivasNaTela.length === 0) setTimeout(spawnGrupo, 400);
            atualizarHUD();
        }
    }
